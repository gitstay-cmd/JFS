...
struct block_t
{
	...
	int (*read)(struct block_t *dev, uint8_t *buf, size_t buf_size, uint32_t sector, uint32_t block_num);
	int (*write)(struct block_t *dev, uint8_t *buf, size_t buf_size, uint32_t sector, uint32_t block_num);
	size_t block_size;
	size_t num_blocks;
