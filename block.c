...
size_t block_read(struct block_t *dev, uint8_t *buf, size_t buf_size, uint32_t starting_block){
	...
	if(dev->supports_multiple_block_read){
    	...
    	return dev->read(dev, buf, buf_size,0,starting_block);
	}
	...
	do {
    	...
    	while(1){
        	...
        	read = dev->read(dev, &buf[buf_offset], to_read, 0,starting_block + block_offset);
...
size_t block_write(struct block_t *dev, uint8_t *buf, size_t buf_size, uint32_t starting_block){
	...
	if(dev->supports_multiple_block_write){
    	dev->write(dev, buf, buf_size, 0,starting_block);
	}
	...
	do{
    	...
    	while(1){
        	...
        	write = dev->write(dev, &buf[buff_offset], to_write, 0,starting_block + block_offset);
